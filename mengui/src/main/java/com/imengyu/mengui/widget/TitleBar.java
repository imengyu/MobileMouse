package com.imengyu.mengui.widget;
...
public class TitleBar extends ConstraintLayout {
  ...
  public void init(Context context, AttributeSet attributeSet){
    ...
    String title = typedArray.getString(R.styleable.TitleBar_title);//标题
    int leftIcon = typedArray.getResourceId(R.styleable.TitleBar_left_icon, R.drawable.ic_back);//左边图片
    int rightIcon = typedArray.getResourceId(R.styleable.TitleBar_right_icon, R.drawable.ic_more);//右边图片
    String rightText = typedArray.getString(R.styleable.TitleBar_right_text);//右边文字
    int titleBarType = typedArray.getInt(R.styleable.TitleBar_title_bar_type, 10);//标题栏类型,默认为10
    boolean titleBarDark = typedArray.getBoolean(R.styleable.TitleBar_title_bar_dark, false);//标题栏类型,默认为10
    typedArray.recycle();
  ...
  public void setRightButtonIconResource(int leftIcon) {
    ivMore.setForeground(ResourcesCompat.getDrawable(getResources(), leftIcon, null));
  }
  public void setLeftButtonIconResource(int rightIcon) {
    ivBack.setForeground(ResourcesCompat.getDrawable(getResources(), rightIcon,null));
  }
  ...
  public void setTitleMode(int titleMode) {
    ...
    if (titleMode == TITLE_MODE_ONE) {//不传入,默认为10,显示更多 文字,隐藏更多图标按钮
      ...
      ivBack.setVisibility(View.VISIBLE);
    } else if(titleMode == TITLE_MODE_ONLY_RIGHT) {
